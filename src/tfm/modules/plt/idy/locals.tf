locals {
  ...
  dc1_waitOneMin = "Start-Sleep -s 60"
  ...
  powershell = "${local.adds_1}; ${local.dns_2}; ${local.dc1_waitOneMin}; ${local.mods_3}; ${local.forest_4}"
  ...
  dc2configure_ad_command = "Install-ADDSDomainController -DomainName ${var.domain.fqdn} -SafeModeAdministratorPassword $password -Credential $mycreds -CreateDnsDelegation:$false -InstallDns:$true -Force:$true"
